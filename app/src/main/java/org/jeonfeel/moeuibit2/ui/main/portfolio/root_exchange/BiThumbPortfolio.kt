...
class BiThumbPortfolio @Inject constructor(
    private val bithumbPortfolioUsecase: BiThumbPortfolioUsecase,
    ...
    private val _marketCodeRes = mutableMapOf<String, BitThumbMarketCodeRes>()
    val marketCodeRes: Map<String, BitThumbMarketCodeRes> get() = _marketCodeRes
    ...
    suspend fun onStop() {
        bithumbPortfolioUsecase.onStop()
    }
    ...
    private suspend fun getMarketCodeRes() {
        ...
        bithumbPortfolioUsecase.fetchMarketCode().collect { res ->
            when (res) {
    ...
    private suspend fun compareUserHoldCoinList() {
        ...
        bithumbPortfolioUsecase.onStart(userHoldCoinsMarkets.split(","))
    }
            ...
            userHoldCoinDtoListPositionHashMap[myCoin?.market]?.let {
                ...
                _userHoldCoinDtoList[it] =
                    UserHoldCoinDTO(
                        ...
                        warning = marketCodeRes[beforeUserHoldCoinDTO.market]?.marketWarning == "WARNING",
                        caution = null,
                            ...
                            result.forEach loop@{
                                ...
                                _userHoldCoinDtoList[position] =
                                    UserHoldCoinDTO(
                                        ...
                                        warning = marketCodeRes[it.market]?.marketWarning == "WARNING",
                                        market = it.market,
                                        isFavorite = 0,
    ...
    private suspend fun getUserHoldCoins() {
        ...
        tempList.addAll(bithumbPortfolioUsecase.getMyCoins())
        myCoinList = tempList
    }
    ...
    suspend fun editUserHoldCoin() {
        ...
        removeCoinInfo.forEachIndexed { index, pair ->
            if (_removeCoinCheckedList[index]) {
                bithumbPortfolioUsecase.removeCoin(pair.first)
            }
        }
    ...
    suspend fun earnReward() {
        val userDao = bithumbPortfolioUsecase.getUserDao()

        if (userDao.getUserByExchange(EXCHANGE_BITTHUMB) == null) {
            userDao.insert(EXCHANGE_BITTHUMB, 10_000_000.0)
        } else {
            userDao.updatePlusMoney(exchange = EXCHANGE_BITTHUMB, 10_000_000.0)
        }

        _userSeedMoney.doubleValue = userDao.getUserByExchange(EXCHANGE_BITTHUMB)?.krw ?: 0.0
    }
    ...
    suspend fun errorReward() {
        ...
        if (userDao.getUserByExchange(EXCHANGE_BITTHUMB) == null) {
            userDao.insert(EXCHANGE_BITTHUMB, 1_000_000.0)
        } else {
            userDao.updatePlusMoney(EXCHANGE_BITTHUMB, 1_000_000.0)
        }
    ...
    suspend fun collectTicker() {
        bithumbPortfolioUsecase.observeTickerResponse().collect { bithumbSocketTickerRes ->
            if (!isPortfolioSocketRunning.value || bithumbSocketTickerRes == null) return@collect

            runCatching {
                if (bithumbSocketTickerRes.code == BTC_MARKET) {
                    _btcTradePrice.doubleValue = bithumbSocketTickerRes.tradePrice
                    if (userHoldCoinDtoListPositionHashMap[BTC_MARKET] == null) return@collect
                }

                if (userHoldCoinDtoListPositionHashMap[bithumbSocketTickerRes.code] == null) return@collect

                val position =
                    userHoldCoinDtoListPositionHashMap[bithumbSocketTickerRes.code] ?: 0
                val tempDto = _userHoldCoinDtoList[position]

                _userHoldCoinDtoList[position] =
                    UserHoldCoinDTO(
                        currentPrice = bithumbSocketTickerRes.tradePrice,
                        openingPrice = bithumbSocketTickerRes.prevClosingPrice,
                        warning = marketCodeRes[bithumbSocketTickerRes.code]?.marketWarning == "WARNING",
                        caution = null,
                        market = bithumbSocketTickerRes.code,
                        isFavorite = 0,
                        myCoinKoreanName = tempDto.myCoinKoreanName,
                        myCoinEngName = tempDto.myCoinEngName,
                        myCoinsQuantity = tempDto.myCoinsQuantity,
